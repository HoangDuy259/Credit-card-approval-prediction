...
import requests
from streamlit_lottie import st_lottie_spinner      #NEW
...
def make_prediction():
    local_model_path = 'saved_models/random_forest/random_forest_model.sav'
    # Load the model from a local file
    with open(local_model_path, 'rb') as fp:
        model = joblib.load(fp)
    # Prediction from the local model
...
if predict_bt:
    ...
    if final_pred[0] == 0:
        str_success = '''You have been approved for a credit card'''
        html_str_success = f"""<p style="background-color: rgb(0,255,50,1);color: rgb(255,255,255,1);font-size:30px;border-radius:7px;line-height:25px;padding-left:12px;padding-top:13px;padding-bottom:13px">{str_success}</p>"""
        st.markdown(html_str_success, unsafe_allow_html=True)
        st.balloons()
    elif final_pred[0] == 1:
        str_fail = '''Unfortunately, you have not been approved for a credit card'''
        html_str_fail = f"""<p style="background-color: rgb(255,0,0,1);color: rgb(255,255,255,1);font-size:30px;border-radius:7px;line-height:25px;padding-left:12px;padding-top:13px;padding-bottom:13px">{str_fail}</p>"""
        st.markdown(html_str_fail, unsafe_allow_html=True)